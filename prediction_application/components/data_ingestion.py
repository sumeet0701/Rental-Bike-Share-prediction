...
class DataIngestion:
    ...
    def download_data(self):
        """
        downloads the zip dataset from the given url and save it to the specified path
        """
        try:
            # extracting thr url to download dataset files
            ...
            # file name  = os.path.basename(download_url)
    ...
    def extract_tgz_file(self, tgz_file_path:str):
        try:
            ...
            # extracting the file from zippeed folder
    ...
    def data_merger_and_split(self):
        try:
            raw_data_dir = self.data_ingestion_config.raw_data_dir # location for extracted data file

            file_name = os.listdir(raw_data_dir)[0]
            data_file_path = os.path.join(raw_data_dir, exist_ok = True)

            # creating collection in mongodb for data dump
            self.db.create_and_check_collection()

            # read each data files and dumping into db
            ...
            # spliting dataset into train and test data based on data indexing 
            ...
            if train_set is not None:
                os.makedir(self.data_ingestion_config.ingested_train_dir, exist_ok= True)
                logging.info(f"Exporting training dataset to file: [{train_file_path}]")
                train_set.to_csv(train_file_path,index=False)

            if test_set is not None:
                os.makedir(self.data_ingestion_config.ingested_test_dir, exist_ok= True)
                logging.info(f"Exporting training dataset to file: [{test_file_path}]")
                train_set.to_csv(test_file_path,index=False)
